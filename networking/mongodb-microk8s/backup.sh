...
if [[ "$NAMESPACE_FOUND" == *"$NAMESPACE"* ]]; then
    ...
    POD_NAME=$(microk8s kubectl -n $NAMESPACE get pod -o jsonpath={.items..metadata.name})
    MONGODB_USER=$(microk8s kubectl -n $NAMESPACE get pod -o jsonpath={.items..spec.containers..env[1]..value})
    MONGODB_DATABASE=$(microk8s kubectl -n $NAMESPACE get pod -o jsonpath={.items..spec.containers..env[2]..value})
    MONGODB_PASSWORD=$(microk8s kubectl -n $NAMESPACE get secret mongodb -o jsonpath="{.data.mongodb-passwords}" | base64 -d | awk -F',' '{print $1}')

    MASTER_PVC=$(microk8s kubectl -n $NAMESPACE get pod $POD_NAME -o jsonpath={.spec.volumes..persistentVolumeClaim.claimName})
    # ex: MASTER_PVC=redis-data-mongodb-master-0
    MASTER_PV=$(microk8s kubectl -n $NAMESPACE get pvc $MASTER_PVC -o jsonpath={.spec.volumeName})
    # ex : MASTER_PV=mongodb-pv-2
    MASTER_PV_HOSTPATH=$(microk8s kubectl -n $NAMESPACE get pv $MASTER_PV -o jsonpath={.spec.hostPath.path})
    ...
    ##
    microk8s kubectl -n $NAMESPACE exec $POD_NAME -- mongodump -d $MONGODB_DATABASE --username=$MONGODB_USER --password=$MONGODB_PASSWORD -o /bitnami/mongodb/dump
    if ! [ $? -eq 0 ]; then
        echo "${red}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
