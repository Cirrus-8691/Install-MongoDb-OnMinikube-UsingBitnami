...
NAMESPACE_FOUND=$(kubectl get namespace | grep $FIND_NAMESPACE)
if [[ "$NAMESPACE_FOUND" == *"$FIND_NAMESPACE"* ]]; then

    POD_NAME=$(kubectl -n $FIND_NAMESPACE get pod -o jsonpath={.items..metadata.name})
    MONGODB_USER=$(kubectl -n $FIND_NAMESPACE get pod -o jsonpath={.items..spec.containers..env[1]..value})
    MONGODB_DATABASE=$(kubectl -n $FIND_NAMESPACE get pod -o jsonpath={.items..spec.containers..env[2]..value})
    MONGODB_PASSWORD=$(kubectl -n $FIND_NAMESPACE get secret mongodb -o jsonpath="{.data.mongodb-passwords}" | base64 -d | awk -F',' '{print $1}')

    MASTER_PVC=$(kubectl -n $FIND_NAMESPACE get pod $POD_NAME -o jsonpath={.spec.volumes..persistentVolumeClaim.claimName})
    # ex: MASTER_PVC=redis-data-mongodb-master-0
    MASTER_PV=$(kubectl -n $FIND_NAMESPACE get pvc $MASTER_PVC -o jsonpath={.spec.volumeName})
    # ex : MASTER_PV=mongodb-pv-2
    MASTER_PV_HOSTPATH=$(kubectl -n $FIND_NAMESPACE get pv $MASTER_PV -o jsonpath={.spec.hostPath.path})
    ...
    kubectl -n $FIND_NAMESPACE exec $POD_NAME -- mongorestore -d $MONGODB_DATABASE --username=$MONGODB_USER --password=$MONGODB_PASSWORD /bitnami/mongodb/dump/$FIND_NAMESPACE/$MONGODB_DATABASE
    if ! [ $? -eq 0 ]; then
        echo "${red}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
